...
class RobotEnvSlide:
    ...
    def reset(self):
        ...
        # 使用相对路径
        self.pandaUid = p.loadURDF("./gym_myrobot/envs/open_manipulator_for_slide.urdf", useFixedBase=True)  # fixedbase 是吧机械臂的底座固定住，不然每次仿真都会乱跑！
        # 重设所有joint的位置
        rest_poses = [0.000, 0.000 ,0.000 ,0.000]   # 前四个joint以及两个finger，要符合joint 的限定范围
        for i in range(4):
