...
for j in range(1):
    ...
    for i in range(500):
        ...
        joints_sim_now.append(envsim.joint_values_now)
        if done:
            break

actions_sim = np.array(actions_sim)
joints_sim_now = np.array(joints_sim_now)
...
plt.title('sim-position line')

from gym_myrobot.envs.real_arm_env import RealarmEnv
envreal = RealarmEnv(reward_type='dense',use_gripper=True, use_fixed_target=True, distance_threshold=0.005)
...
plt.title('real-sim-position line')


# 比对joints delta
plt.subplot(2,2,4)
plt.plot(t, actions_real[:,0],'r--', t,actions_real[:,1],'b--', t, actions_real[:,2],'g--', t, actions_real[:,3],'y--' ,
         t, actions_sim[:,0],'r', t,actions_sim[:,1],'b', t, actions_sim[:,2],'g', actions_sim[:,3],'y')
plt.xlabel('time-step')
plt.ylabel('actions')
plt.legend(['joint1_real','joint2_real','joint3_real','joint4_real', 'joint1_sim', 'joint2_sim', 'joint3_sim','joint4_sim'], loc='best')
plt.title('real-sim-delta-joint line')
plt.show()

# 展示joints
# 比对joints delta
plt.figure()
plt.plot(t, joints_sim_now[:,0],'r', t,joints_sim_now[:,1],'b', t, joints_sim_now[:,2],'g', joints_sim_now[:,3],'y')
plt.xlabel('time-step')
plt.ylabel('actions')
plt.legend([ 'joint1_sim', 'joint2_sim', 'joint3_sim','joint4_sim'], loc='best')
plt.title('sim-joint line')
plt.show()